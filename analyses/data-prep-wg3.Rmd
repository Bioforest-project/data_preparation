...
```{r paracou-vername}
FGSites = c("Paracou", "Bafog", "Montagne_Tortue", "Manare") 
if (any(data$Site %in% FGSites)) {
  
  if (!require(vernabota)) remotes::install_github("EcoFoG/vernabota", build_vignettes = TRUE); library(vernabota)
  
  dataFG = subset(data, Site %in% FGSites)
  dataFG[, latin := paste(Genus, tstrsplit(Species, "_")[[1]])]
  
  # replace names of undetermined species with NA
  
  dataFG[Species == "Indet.", latin := NA]
  
  # prepare data
  dataVern <- data.table(
    Site = dataFG$Site,
    Plot = dataFG$Plot,
    CodeAlive = dataFG$Original_CodeAlive, 
    idTree = dataFG$IdTree,  
    BotaCertainty = 4 - 
      2*grepl("Indet", dataFG$Species) -
      2*grepl("Indet", dataFG$Genus), ## temp
    BotaSource = ifelse(grepl("Indet", dataFG$Genus), "Vern", "Bota"),
    VernName = dataFG$VernName, 
    Family = dataFG$Family, 
    Genus = dataFG$Genus, 
    Species = dataFG$Species
  ) |> vernabota::PrepData()
    ...
    rbindlist(
      lapply(seq_len(nrow(options)), function(i) {
        rbindlist(suppressWarnings(vernabota::SimFullCom(
          Data2fill = dataVern[Site == "Paracou" & Plot == options$Plot[i]], 
          prior = get(options$Prior[i]), 
          wp = 0.25, 
          NSim = 1, 
          eps = 0.01,
          Determ = TRUE
        ...
        # other sites
        rbindlist(
          lapply(FGSites[FGSites!="Paracou"], function(s) {
            rbindlist(suppressWarnings(vernabota::SimFullCom(
              Data2fill = dataVern[Site == s], 
              prior = PriorAllFG, 
              wp = 0.25, 
              NSim = 1, 
              eps = 0.01,
  ...
  data[Site %in% FGSites, c("Genus", "Species") := tstrsplit(GensSpCor, "-")[1:2]]
  data[,GensSpCor := NULL]
}

```

Levels of species identification per site 

```{r fig.width=8, fig.height=4}
dataIndet = data[, .(
  pIndet_Sp = sum(grepl("undet|indet|sp\\.|sp$|spp.|spp.|L.", tolower(Species)) | is.na(Species))/ length(Species), 
  pIndet_Gen = sum(grepl("undet|indet", tolower(Genus)) | is.na(Genus))/ length(Genus)), 
  .(Site, Year)] 

melt(dataIndet, id.vars = c("Site", "Year")) |> 
  ggplot(aes(x = Year, color = Site, y = value)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(~variable)
```
