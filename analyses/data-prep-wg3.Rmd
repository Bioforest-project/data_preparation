...
dataPlot = 
  rbind(
    ...
    data[, .(
      stock = Diversity(as.vector(table(ScientificName)), q=1, CheckArguments= FALSE) |> suppressWarnings(), 
      mortality = Diversity(as.vector(table(ScientificName[status == "M"])), q=1, CheckArguments= FALSE) |> suppressWarnings(), 
      recruitment = Diversity(as.vector(table(ScientificName[status == "R"])), q=1, CheckArguments= FALSE) |> suppressWarnings(), 
      growth = NA, 
      meas = "div1_n"
    ), .(Site, Plot, Year, class)]
  )
